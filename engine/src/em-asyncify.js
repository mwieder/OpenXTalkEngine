...
mergeInto(LibraryManager.library, {
	...
	$LiveCodeAsync: {
		...
		resume: function(delayed) {
			...
			// Call the resume function, passing the return value
			// which must be a number.
			resume(LiveCodeAsync.isTimedOut() ? 0 : 1);
		},
		...
		pause: function(timeout) {
			...
			// the closure passed to Asyncify.handleSleep() will
	...
	// Run hooks once engine has resumed
	MCEmscriptenAsyncRunHooks__deps: ['$LiveCodeAsync'],
	MCEmscriptenAsyncRunHooks: function() {
		// Run pre-resume hooks
